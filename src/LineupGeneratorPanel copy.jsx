          ...
          {activeTab === 'PRIZEPICKS' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-1">Mix Type</label>
                  <select value={mixType} onChange={(e) => setMixType(e.target.value)} className="w-full rounded-md px-4 py-2 border border-gray-300">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">Max Lineups</label>
                  <input type="number" value={maxLineups} onChange={(e) => setMaxLineups(e.target.value)} className="w-full rounded-md px-4 py-2 border border-gray-300" />
                </div>

                <div>
                  <label className="block text-sm font-semibold mb-1">Confidence</label>
                  <select value={confidenceFilter} onChange={(e) => setConfidenceFilter(e.target.value)} className="w-full rounded-md px-4 py-2 border border-gray-300">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">Home/Away</label>
                  <select value={homeAwayFilter} onChange={(e) => setHomeAwayFilter(e.target.value)} className="w-full rounded-md px-4 py-2 border border-gray-300">
                    <option value="">All</option>
                ...
                <div className="lg:col-span-2">
                  <label className="block text-sm font-semibold mb-1">Filter by Tag</label>
                  <select multiple value={lineupTagFilter} onChange={(e) => setLineupTagFilter(Array.from(e.target.selectedOptions, option => option.value))} className="w-full h-40 rounded-md px-4 py-2 border border-gray-300">
                    {tags.map(tag => <option key={tag} value={tag}>{tag}</option>)}
                  </select>
                </div>

                <div className="lg:col-span-2">
                  <label className="block text-sm font-semibold mb-1">Filter by Game</label>
                  <select multiple value={lineupGameFilter} onChange={(e) => setLineupGameFilter(Array.from(e.target.selectedOptions, option => option.value))} className="w-full h-40 rounded-md px-4 py-2 border border-gray-300">
