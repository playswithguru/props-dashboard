...
export default function PropsDashboard() {
  ...
  const [homeAwayFilter, setHomeAwayFilter] = useState("Both");
  ...
  const handleLineupGenerate = async () => {
    const config = {
      ...
      filterGame: lineupGameFilter,
      homeAwayFilter: homeAwayFilter,  // Add the homeAwayFilter
    };
  ...
  const filteredProps = propsData.filter(p => {
    ...
    const isHomeOrAwayMatch = homeAwayFilter === "Both" || 
      (homeAwayFilter === "Home" && p.Team === teamA) || 
      (homeAwayFilter === "Away" && p.Opponent === teamB);
